...
def edit_profile():
    ...
    if request.method == 'POST':
        ...
        # populate the user data from the database
...
@routes_blueprint.route('/enroll/<classid>', methods=['GET', 'POST'])
@login_required
def enroll(classid):
    theclass = Class.query.filter_by(id = classid).first()
    if theclass is None:
...
@routes_blueprint.route('/unenroll/<classid>', methods=['GET', 'POST'])
@login_required
